#include "ray/utah_teapot.h"

u32 utah_teapot_patches [UTAH_TEAPOT_NUM_PATCHES][16] = {
...
f32 utah_teapot_vertices[UTAH_TEAPOT_NUM_VERTICES][3] = {
...
{ 
	...
	u_curve[0] = eval_bezier_curve(control_p + 4 * 0, u);
	u_curve[1] = eval_bezier_curve(control_p + 4 * 1, u);
	u_curve[2] = eval_bezier_curve(control_p + 4 * 2, u);
	u_curve[3] = eval_bezier_curve(control_p + 4 * 3, u);
...
deirv_u_bezier(Vec3 control_p[static 16], f32 u, f32 v)
{
    ...
    p[2] = control_p[8 + 0]; 
    p[3] = control_p[12 + 0]; 
    v_curve[0] = eval_bezier_curve(p, v); 
    
    p[0] = control_p[1]; 
    p[1] = control_p[4 + 1]; 
    p[2] = control_p[8 + 1]; 
    p[3] = control_p[12 + 1]; 
    v_curve[1] = eval_bezier_curve(p, v); 
    
    p[0] = control_p[2]; 
    p[1] = control_p[4 + 2]; 
    p[2] = control_p[8 + 2]; 
    p[3] = control_p[12 + 2]; 
    v_curve[2] = eval_bezier_curve(p, v); 
    
    p[0] = control_p[3]; 
    p[1] = control_p[4 + 3]; 
    p[2] = control_p[8 + 3]; 
    p[3] = control_p[12 + 3]; 
    v_curve[3] = eval_bezier_curve(p, v);
    
    Vec3 result = deriv_bezier(v_curve, u);
    return result; 
}

Vec3 
deriv_v_bezier(Vec3 control_p[static 16], f32 u, f32 v)
{
    Vec3 u_curve[4];
    u_curve[0] = eval_bezier_curve(control_p + 4 * 0, u);
    u_curve[0] = eval_bezier_curve(control_p + 4 * 1, u);
    u_curve[0] = eval_bezier_curve(control_p + 4 * 2, u);
    u_curve[0] = eval_bezier_curve(control_p + 4 * 3, u);
    
    Vec3 result = deriv_bezier(u_curve, v);
    return result;
}
